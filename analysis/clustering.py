...
def load_queue():
    ...
    required_cols = ["id", "fileName", "driveFileId", "mainLabel"]
...
def build_feature_dataframe(queue_df):
    rows = []
...
def analyze(color_df):
    ...
    if missing_cols:
        raise ValueError(f"特徴量列がありません: {missing_cols}")
...
def save_cluster_plot(color_df, run_dir):
    ...
    for cluster_id in sorted(color_df["cluster"].unique()):
        sub = color_df[color_df["cluster"] == cluster_id]
    ...
    plt.savefig(latest_path, dpi=160)
    plt.savefig(run_path, dpi=160)
    plt.show()
...
def main():
    ...
    queue_df = load_queue()
    color_df = build_feature_dataframe(queue_df)
